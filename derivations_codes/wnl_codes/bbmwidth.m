...
B1 = -0.05*H*0.25*(tanh((x+0.1*xmax)/(0.0125*xmax))-tanh((x-0.1*xmax)/(0.0125*xmax)));
B2 = -0.05*H*0.25*(tanh((x+0.05*xmax)/(0.0125*xmax))-tanh((x-0.05*xmax)/(0.0125*xmax)));
u0=B1;
u02=B2;
...
%           'DefaultAxesFontWeight','bold');
   subplot(2,1,1)
   h=plot(x/H,u0/H,'k:',x/H,B1/H,'k-')
   set(h,'linewidth',2)
   grid on
   ylabel('B/H');
   title(['time = ' num2str(t,2)]);
   axis([xmin/H xmax/H -1.875*max(max(abs(u0)))/H 0.125*max(max(abs(u0)))/H])
   subplot(2,1,2)
   h=plot(x/H,u02/H,'k:',x/H,B2/H,'k--')
   set(h,'linewidth',2)
   grid on
   ylabel('B/H');
   title(['time = ' num2str(t,2)]);
