...
B1 = -0.175*H*sech((x+0.1*xmax)/(0.00625*xmax)).^2;
B1=B1-0.05*H*sech((x-0.1*xmax)/(0.0125*xmax)).^2;
B1=B1-0.05*H*sech((x-0.2*xmax)/(0.0125*xmax)).^2;
B1=B1-0.05*H*sech((x-0.3*xmax)/(0.0125*xmax)).^2;
B2 = -0.175*H*sech((x+0.1*xmax)/(0.00625*xmax)).^2;%.*sin(2*pi*x/2);
u0=B1;
...
%           'DefaultAxesFontWeight','bold');
   %subplot(2,1,1)
   h=plot(x/H,B1/H,'b-',x/H,B2/H,'r-')
   set(h,'linewidth',2)
   grid on
   xlabel('x/H');
   ylabel('B/H');
   title(['time = ' num2str(t,2)]);
  % axis([xmin xmax -1*max(max(abs(u0))) 1.0*max(max(abs(u0)))]) % for positive polarity
   axis([xmin/H xmax/H -1.25*max(max(abs(u0)))/H 0.05*max(max(abs(u0)))/H]) % for
  % negative polarity
   drawnow
